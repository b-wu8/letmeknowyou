####################################################
### MODEL VGG_net
### By adjusting the number of weighted layers we can have different VGG net structure
### in our project VGG16 is used
####################################################
...
https://github.com/usef-kh/fer/blob/master/models/vgg.py
"""
class VGG(torch.nn.Module):
    ...
    def __init__(self, num_classes, height, width):
        ...
        self.block_1 = torch.nn.Sequential(
                torch.nn.Conv2d(in_channels=3, # input channel = 3
                                out_channels=64,
                                kernel_size=(3, 3),
                                stride=(1, 1),
                                padding=1),
        ...
        self.classifier = torch.nn.Sequential(
            ...
            torch.nn.ReLU(True),
            # torch.nn.Dropout2d(p=0.5), # discarded after the 1st run
...
if __name__ == "__main__":
    ...
    #     tt.ToTensor(),
    #     tt.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))]) # normalize so that the pictures centered at 0. --> ?
    #
    #
    ...
    train_transform = tt.Compose([tt.Resize((128, 128)), tt.ToTensor()]) # upscale the pictures since VGG16 has too many more layers for small inputs
    valid_transform = tt.Compose([tt.Resize((128, 128)), tt.ToTensor()])
    ...
    model_vgg = VGG(num_classes=len(expression),height=4,width=4)
    ...
    for i in tqdm(range(epoch)):
        ...
        optimizer = torch.optim.Adam(model_vgg.parameters(), lr=learning_rate)
        score = 0
        model_vgg.train()
        for images, labels in train_loader: # batch size 512
            out = model_vgg(images)
            loss = loss_function(out, labels)
            model_vgg.zero_grad() # Sets the gradients of all optimized torch. Tensors to zero.
    ...
    torch.save(model_vgg, MODEL_DIR+"VGGNet_model_bs64_lr0001.pt")
